...
POSITIVE_TAIL = pd.Series(CAUCHY_RVS[CAUCHY_RVS > 0], name='samples')
NEGATIVE_TAIL = pd.Series(-CAUCHY_RVS[CAUCHY_RVS < 0], name='samples')

QUANTILE, METHOD = 0.99, 'bounded'
RESULT1 = distributions.Pareto.fit(POSITIVE_TAIL, scale=None,
                                   quantile=QUANTILE, method=METHOD)
RESULT2 = distributions.Pareto.fit(NEGATIVE_TAIL, scale=None,
                                   quantile=QUANTILE, method=METHOD)
...
def test_cauchy_estimation(size=0.01):
    ...
    testing.utilities.test_scaling_exponent_estimation(DESIRED_ALPHA, RESULT1, size)
    testing.utilities.test_scaling_exponent_estimation(DESIRED_ALPHA, RESULT2, size)


def test_cauchy_goodness_of_fit(size=0.05):
    """Test the goodness of fit of the Pareto distribution."""
    test_kwargs = {'seed': None, 'result': RESULT1, 'data': POSITIVE_TAIL,
    ...
    test_kwargs = {'seed': None, 'result': RESULT2, 'data': NEGATIVE_TAIL,
                   'scale': None, 'quantile': QUANTILE, 'method': METHOD}
    pvalue, _ = distributions.Pareto.test_goodness_of_fit(**test_kwargs)
    assert pvalue > size, "Goodness of fit test Type I error!"
